...
def _reshape_pad(X: np.ndarray, frequency: int):
    ...
    if r == 0:
        ...
        diff = frequency-r
        X_out = np.pad(array=X, pad_width=(diff, 0), mode='constant',
                       constant_values=0).reshape((frequency, -1), order='F')
        return X_out
...
def _mad_outlier(X: np.ndarray):
    ...
    Output = np.apply_along_axis(
        _decision_mad, axis=0, arr=X.reshape(1, -1), mu=mad, ma=median)
    return Output
...
class AnomalyDetection_RPCA(BaseEstimator):
    ...
    def _fit(self, X):
        ...
        if self.Spenalty == -1:
            self._Spenalty = 1.4 / \
                sqrt(max(self.frequency, len(X)/self.frequency))
        ...
        if (self.verbose):
            ...
            print("..........Start Process..........")
            print("Time Series, frequency=%d and Num Periods= %d." %
                  (M.shape[0], M.shape[1]))
            self._L, self._S, self._E = rpca(
                M, Lpenalty=self.Lpenalty, Spenalty=self._Spenalty, verbose=self.verbose)
        else:
            self._L, self._S, self._E = rpca(
                M, Lpenalty=self.Lpenalty, Spenalty=self.Spenalty, verbose=self.verbose)
    ...
    def fit_transform(self, X):
        ...
        """
        self.fit(X)
        L_transform = (self._L.T.reshape((-1, 1)).ravel()
                       * self.global_sdt)+self.global_mean
    ...
    def to_frame(self, X, add_mad=True):
        ...
        if length_diff > 0:
            X = np.pad(array=X, pad_width=(length_diff, 0),
    ...
    def plot(self, figsize=(10, 6)):
        ...
        ax.plot(range(len(L_transform,)), L_transform+E_transform,
                label='Time Serie rPCA', ls='--', c='royalblue')
        ax.plot(range(len(L_transform)), np.abs(
            S_transform), c='red', label='Outliers')
